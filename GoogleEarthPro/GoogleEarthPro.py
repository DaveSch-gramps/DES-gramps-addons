...
GoogleEarthPro  kml or kmz file format.
"""
...
import codecs
import logging
LOG = logging.getLogger(".GoogleEarthPro")
...
if os.sys.platform == 'win32':
    FILE_PATH = r'"%s\Google\Google Earth Pro\googleearth.exe"'\
    ...
    if not _GOOGLEEARTH_OK:
        ...
        FILE_PATH = r'"%s\Google\Google Earth Pro\client\googleearth.exe"'\
                    % (os.getenv('ProgramFiles'))
        NORM_PATH = os.path.normpath(FILE_PATH)
        _GOOGLEEARTH_OK = search_for(NORM_PATH)
...
class GoogleEarthPro(MapService):
    """
